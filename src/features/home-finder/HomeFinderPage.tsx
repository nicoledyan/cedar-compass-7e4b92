...
export default function HomeFinderPage() {
  ...
  return <main className="finder-page"><div className="finder-shell">
    ...
    {signedInEmail && <details className="finder-preferences"><summary>What I’m looking for <span>Edit AI criteria</span></summary><div><p>Add, remove, or rewrite anything. These criteria are saved on this device and used for every new score.</p><textarea value={preferences} onChange={(event) => setPreferences(event.target.value)} rows={10}/><small>Changes apply the next time you add a listing or rerun a review.</small></div></details>}
...
function HomeCard({ home, preferences, editing, autoAnalyze, onEdit, onUpdate, onDelete }: { home: HomeRecord; preferences: string; editing: boolean; autoAnalyze: boolean; onEdit: () => void; onUpdate: (home: HomeRecord) => void; onDelete: () => void }) {
  const complete = (analysis: AiHomeAnalysis) => onUpdate({ ...home, aiScore: analysis.fitScore, aiVerdict: analysis.verdict, aiSummary: analysis.summary, aiObservations: analysis.observations, aiCautions: analysis.cautions, aiSources: analysis.sources });
