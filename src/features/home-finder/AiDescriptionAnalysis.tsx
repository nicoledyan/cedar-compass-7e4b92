...
export default function AiDescriptionAnalysis({ home, preferences, autoAnalyze = false, onComplete }: { home: HomeRecord; preferences: string; autoAnalyze?: boolean; onComplete: (analysis: AiHomeAnalysis) => void }) {
  ...
  return <section className="finder-ai-simple" aria-live="polite">
    ...
    {analysis && !loading && <div className="finder-ai-summary">
      ...
      <div className="finder-review-hero"><div className="finder-ai-big-score"><strong>{analysis.fitScore}</strong><span>fit score</span></div><div className="finder-ai-intro"><p className="finder-ai-verdict">{analysis.verdict}</p><h3>{analysis.summary}</h3><div className="finder-confidence"><ShieldCheck size={16}/><span><strong>{analysis.confidence}% evidence confidence</strong><em>How complete the research is—not how good the home is.</em></span></div></div></div>
